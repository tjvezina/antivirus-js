...
let MAX_SHIP_ROT: number;
let worldRot: number;

let shipRotZ = 0;

let projectileList: ProjectileActor[] = [];

let incomingList: IncomingActor[] = [];

let explosions: ParticleExplosion[] = [];

const SPAWN_INC_INTERVAL = 12.5; // How often the difficulty increases
const SPAWN_INC_AMOUNT = 0.025; // How much the spawn time increases by
let lastSpawnInc = 0;
let closestEnemy = TUBE_LENGTH; // Tube color changes based on how close enemies are
let firstEnemyFound = false;

const INCOMING_SPEED_INC = 0.75; // How much to increase enemy speed by
...
/* IMAGES */
let imgPaused: p5.Image;
let imgMainScreen: p5.Image;
let imgGUILeft: p5.Image;
let imgGUIRight: p5.Image;
let imgGreenBar: p5.Image;
let imgSpeaker: p5.Image;
let imgSpeakerBar: p5.Image;
let imgTutorial: p5.Image;
let imgClick2Go: p5.Image;
let imgGameOver: p5.Image;
let imgYouWin: p5.Image;
...
let music1: p5.SoundFile;
let music2: p5.SoundFile;
let music3: p5.SoundFile;

/* SFX */
let shoot: p5.SoundFile;
let pickupGood: p5.SoundFile;
let pickupBad: p5.SoundFile;
let balloonPop: p5.SoundFile;
let blowup: p5.SoundFile;
let alarm: p5.SoundFile;
...
let tutorialShip: Mesh;
let tutorialShot: Mesh;
let tutorialPickup: Mesh;
let tutorialData: Mesh;
let tutorialEnemy: Mesh;
...
let currentMusic: p5.SoundFile;

const END_GAME_DELAY = 3;
let endGameTime = 0;

// When hitting ESC to exit pointer lock, the mouse jumps back to where it was when pointer lock began,
// and one last `draw()` loop completes with that potentially huge `movedX` value before the pointer lock
// event is fired and the game pauses; my only idea to avoid this is use last frame's `movedX` value
let prevMovedX = 0;
...
globalThis.setup = function (): void {
  ...
  volume = (getItem('volume') as number | null) ?? 0.4;
...
globalThis.keyPressed = function (): void {
...
globalThis.mouseClicked = function (): void {
...
globalThis.mousePressed = function (): void {
...
function mainMenu(): void {
  ...
  if (isTutorial) {
    ...
    if (!isTitleScreen && !isTransition) {
      ...
      tutorialShip.world = Matrix.mult(
        Matrix.createTranslation(-32, 20, 0),
        Matrix.createRotationY(radians(-10)),
        Matrix.createRotationX(radians(-90)),
      );
      tutorialShot.world = Matrix.mult(
        Matrix.createTranslation(6.5, 11.4, 0),
        Matrix.createRotationY(tutorialMeshSpin),
      );
      tutorialPickup.world = Matrix.mult(
        Matrix.createTranslation(1.5, 11, 0),
        Matrix.createRotationY(tutorialMeshSpin),
      );
      tutorialData.world = Matrix.mult(
        Matrix.createTranslation(-24, -6, 0),
        Matrix.createRotationY(radians(10)),
        Matrix.createScale(2),
      );
      tutorialEnemy.world = Matrix.mult(
        Matrix.createTranslation(24, 3, 0),
        Matrix.createRotationX(radians(-10)),
        Matrix.createRotationY(radians(10)),
        Matrix.createScale(2),
      );
...
function update(): void {
  ...
  shipMesh.world = Matrix.mult(
    Matrix.createTranslation(0, -18, 0),
    Matrix.createRotationZ(shipRotZ),
  );
  ...
  if (!firstEnemyFound) closestEnemy = TUBE_LENGTH;
  ...
  for (const projectile of [...projectileList]) {
    for (const incoming of [...incomingList]) {
      if (incoming.isLeaving) continue;
...
function drawGame(): void {
  // NOTE: No Z-buffering means the draw order is very important!
  ...
  // Traverse the incoming mesh order backwrads to make sure the farthest meshes are drawn first
...
function checkCollision(a: ProjectileActor, b: IncomingActor): boolean {
...
function playSound(sound: p5.SoundFile): void {
...
function playMusic(music: p5.SoundFile): void {
  currentMusic?.stop();
  currentMusic = music;
...
// Takes a number and rounds it to a value which lines up with
// one of the 18 lanes that form the main tunnel
function roundToTrack(value: number): number {
  let deg = floor(degrees(value));
  while (deg < 0) deg += 360;
  return radians(deg - (deg % 20) + 10);
}

// Takes the worldRot value and determines the angle to set the square
// panel which highlights which lane the player is aiming down
function getPanelPos(worldRot: number): number {
